...
int main()
{
	...
	c = fork();
        ...
        else
        {
                if(c==0)
                {
			const char *argv[] = {"ls", "-Rl", nullptr};
			printf("Execution using execvp() system call\n");
                	execvp("/bin/ls",const_cast<char *const *>(argv));
                	exit(1);
                }
        }
	wait(NULL);
...
/*
Execution using execvp() system call
.:
total 144
...
-rw-rw-r-- 1 lazyuser lazyuser  2782 Aug 31 12:46 27d.c
-rw-rw-r-- 1 lazyuser lazyuser   886 Aug 31 12:48 27e.c
